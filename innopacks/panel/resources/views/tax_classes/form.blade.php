  ...
  <div class="card-body">
    <form class="needs-validation" novalidate id="app-form"
      ...
      <div class="wp-500 m-auto">
        <x-common-form-input title="الاسم" name="name" value="{{ old('name', $tax_class->name) }}" required placeholder="الاسم" />
        <x-common-form-input title="الوصف" name="description" value="{{ old('description', $tax_class->description) }}" required placeholder="الوصف" />
        <x-panel::form.row title="قاعدة" required>
          @php ($index = 0)
          <table class="table table-bordered">
            <thead>
              <tr>
                <th>معدل الضريبة</th>
                <th>بناء على العنوان</th>
                <th>أولوية</th>
                <th class="text-end">جاري العمل</th>
              </tr>
            </thead>
            <tbody>
                ...
                <tr>
                  ...
                  <td>
                    <input type="text" name="tax_rules[{{ $index }}][priority]" class="form-control form-control-sm"
                           value="{{ $rule->priority }}" placeholder="أولوية">
                  </td>
                  <td class="text-end">
    ...
    $('.add-tax').click(function () {
      var html = `
        <tr>
          ...
          <td>
          <input type="text" name="tax_rules[{{ $index }}][priority]" class="form-control form-control-sm" value="0" placeholder="أولوية">
          </td>
          <td class="text-end">
            <button type="button" class="btn btn-sm btn-outline-danger remove-tax">{{ __('panel/common.delete')}}</button>
          </td>
